...
@property (nonatomic, strong) NSMutableArray * saveDeleteImage;;
@end

@implementation ZLShowBigImgViewController

-(NSMutableArray *)saveDeleteImage
{
    if (_saveDeleteImage == nil) {
        
        _saveDeleteImage =[NSMutableArray array];
    }
    return _saveDeleteImage;
}
...
- (void)navRightBtn_Click:(UIButton *)btn
{
    ...
    NSLog(@"%ld  %@",_currentPage,self.models);
    ...
    if (!btn.selected) {
        ...
        //移除
        model.isSelected = NO;
       // [self.saveDeleteImage addObject:model];
        
        NSLog(@"%@ %ld",_saveDeleteImage,_saveDeleteImage.count);
