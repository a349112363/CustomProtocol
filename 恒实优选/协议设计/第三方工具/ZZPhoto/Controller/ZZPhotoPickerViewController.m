...
- (UIBarButtonItem *)backBtn{
    if (!_backBtn) {
        ...
        [back_btn setTitle:@"相册" forState:UIControlStateNormal];
        [back_btn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        // [back_btn setImage:[UIImage imageNamed:@"style_back.png"] forState:UIControlStateNormal];
        back_btn.frame = CGRectMake(0, 0, 50, 44);
...
-(void)done{
    ...
    if ([self.selectArray count] == 0) {
        ...
        //将提示添加在当前view 上 防止卡死用户不能点击取消关闭
        [self.view addSubview:[ZZPhotoHud sharedHud]];
        [[ZZPhotoHud sharedHud] show];
...
//             QFLOG(@"%@",photo.imageUrl);
//            
//            PHAsset *phAsset = (PHAsset *)photo.asset;
//            
...
//                    image = result;
//                
//                    [[AppDelegate appdelegate].SaveImages insertObject:result atIndex:0];
//
//                    //NSURL *imageUrl = (NSURL *)[info objectForKey:@"PHImageFileURLKey"];
//                    //complection(result,imageUrl);
//                    
//                    if ([AppDelegate appdelegate].SaveImages.count < self.selectArray.count){
//                        return;
//                    }
////                    if (self.PhotoResult) {
////                        
////                        self.PhotoResult([AppDelegate appdelegate].SaveImages);
////                    }
//                    
//                     QFLOG(@"567");
//                    
//                    for (int i = 0; i < [AppDelegate appdelegate].SaveImages.count; i++) {
//                        
//                        UIImageView * imageview =[[UIImageView alloc]initWithFrame:CGRectMake(i * (IPHONE_WIDTH / 6), 64, IPHONE_WIDTH / 6, 200)];
//                        
//                        imageview.backgroundColor =[UIColor redColor];
//                        imageview.image = [AppDelegate appdelegate].SaveImages[i];
//                        
//                        [[AppDelegate appdelegate].window addSubview:imageview];
//                    }
//                    
//                    [[ZZPhotoHud sharedHud] removeHudUI];
//                    [self dismissViewControllerAnimated:YES completion:nil];
//                    
//                }
//                
            ...
            [self.datas GetImageObject:photo.asset complection:^(UIImage *image,NSURL *imageUrl) {
                ...
                if (image){
                    ...
                    if (self.width) {
                         ...
                         image = [UIImage scaleFromImage:image toSize:CGSizeMake(self.width, self.height)];
